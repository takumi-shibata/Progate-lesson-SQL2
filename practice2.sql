# 1.DISTINCTとは: SELECTで取得するカラムに使用することで、そのカラム内の重複するデータを省くことができる「SELECT DISTINCT (カラム名)」
...
# 2.四則演算: SQLでも四則演算が可能。SELECTで取得するカラムに使用することで計算後のデータを取得できる
...
# 3.関数SUM: SQLでカラム内の数値の「合計」を計算する場合は「SUM」を用いる。
...
# 4.関数AVG: SQLでカラム内の数値の「平均」を計算する場合は「AVG」を用いる。
...
# 5.関数COUNT: 指定したカラムのデータの合計数を取得する。「SELECT COUNT(カラム名)」
...
# 6.関数MAX/MIN: SQLでカラム内の「最大のデータ」を知りたい時は「MAX」、「最小のデータ」を知りたい時は「MIN」を用いる。
...
# 7.GROUP BYとは: データをグループ化することができる。「GROUP BY カラム名」とすることで、指定したカラム内で同一のデータを持つレコード同士が同じグループとなる
...
# 8.複数カラムを指定したGROUP BY:「GROUP BY カラム名1,カラム名2」カラム名同士をコンマ(,)で繋げる
...
# 10.HAVINGとは: GROUP BYでグループ化したデータから、更に特定のグループのみを取得したい時に用いる「GROUP BY カラム名 HAVING 条件」
